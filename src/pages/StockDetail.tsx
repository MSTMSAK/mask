...
import {
  TrendingUp, TrendingDown, ArrowLeft, Download, Activity,
  Newspaper, MessageCircle, DollarSign, FileText,
} from 'lucide-react';
...
export default function StockDetail() {
    ...
    return (
      <div className="min-h-screen bg-[#0B0E14]">
        ...
        <div className="pt-24 flex flex-col items-center justify-center h-[60vh]">
          ...
          <button
            onClick={() => navigate('/')}
            className="px-4 py-2 rounded-lg bg-[#164E63] text-[#06B6D4] text-sm"
          >
            返回首页
          </button>
        </div>
  ...
  const dimensionDetails = [
    {
      key: 'fundamental', label: '基本面', score: fiveD.fundamental,
      icon: Activity, color: '#06B6D4',
      details: [
        { name: '估值水平', value: price?.pe ? `${price.pe.toFixed(1)}` : '--', desc: price?.pe && price.pe < 30 ? '低估区间' : price?.pe && price.pe < 50 ? '合理区间' : '偏高关注' },
        { name: '市值', value: price?.marketCap ? `${(price.marketCap / 1e8).toFixed(0)}亿` : '--', desc: '总市值' },
        { name: '基本面评分', value: `${fiveD.fundamental}分`, desc: fiveD.fundamental >= 80 ? '优秀' : fiveD.fundamental >= 60 ? '良好' : '一般' },
      ],
    },
    {
      key: 'technical', label: '技术面', score: fiveD.technical,
      icon: TrendingUp, color: '#10B981',
      details: [
        { name: '涨跌幅', value: `${price?.changePercent && price.changePercent > 0 ? '+' : ''}${price?.changePercent?.toFixed(2) || 0}%`, desc: price?.changePercent && price.changePercent > 0 ? '上涨' : '调整' },
        { name: '换手率', value: `${price?.turnoverRate?.toFixed(2) || 0}%`, desc: price?.turnoverRate && price.turnoverRate > 5 ? '活跃' : '平稳' },
        { name: '技术面评分', value: `${fiveD.technical}分`, desc: fiveD.technical >= 80 ? '强势' : fiveD.technical >= 60 ? '中性' : '弱势' },
      ],
    },
    {
      key: 'capital', label: '资金面', score: fiveD.capital,
      icon: DollarSign, color: '#F59E0B',
      details: [
        { name: '成交量', value: price?.volume ? `${(price.volume / 1e4).toFixed(0)}万手` : '--', desc: '成交额' },
        { name: '换手率', value: `${price?.turnoverRate?.toFixed(2) || 0}%`, desc: '资金参与度' },
        { name: '资金面评分', value: `${fiveD.capital}分`, desc: fiveD.capital >= 80 ? '资金活跃' : fiveD.capital >= 60 ? '资金平稳' : '资金流出' },
      ],
    },
    {
      key: 'news', label: '消息面', score: fiveD.news,
      icon: Newspaper, color: '#8B5CF6',
      details: [
        { name: '涨跌驱动', value: price?.changePercent && price.changePercent > 3 ? '利好驱动' : '正常波动', desc: '消息影响' },
        { name: '消息面评分', value: `${fiveD.news}分`, desc: fiveD.news >= 80 ? '利好密集' : fiveD.news >= 60 ? '消息中性' : '利空居多' },
        { name: '舆情风险', value: fiveD.news >= 70 ? '较低' : fiveD.news >= 40 ? '中等' : '较高', desc: '风险评估' },
      ],
    },
    {
      key: 'sentiment', label: '舆情面', score: fiveD.sentiment,
      icon: MessageCircle, color: '#EC4899',
      details: [
        { name: '市场情绪', value: fiveD.sentiment >= 70 ? '积极' : fiveD.sentiment >= 50 ? '中性' : '谨慎', desc: '投资者情绪' },
        { name: '舆情评分', value: `${fiveD.sentiment}分`, desc: fiveD.sentiment >= 80 ? '情绪高涨' : fiveD.sentiment >= 60 ? '情绪稳定' : '情绪低迷' },
        { name: '关注度', value: fiveD.sentiment >= 70 ? '高' : fiveD.sentiment >= 50 ? '中' : '低', desc: '市场关注度' },
      ],
    },
  ];
          ...
          <ScrollReveal>
            <div className="bg-[#131821] border border-[#1E293B] rounded-2xl p-6 sm:p-8 mb-8 glow-border">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <span className="data-number text-3xl text-[#E2E8F0]">
                      {price?.price ? `¥${price.price.toFixed(2)}` : '--'}
                    </span>
                    <span className={`flex items-center text-sm font-medium ${
                      (price?.changePercent || 0) >= 0 ? 'text-[#F43F5E]' : 'text-[#10B981]'
                    }`}>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {[
                  { label: '总市值', value: price?.marketCap ? `${(price.marketCap / 1e8).toFixed(0)}亿` : '--' },
                  { label: '市盈率', value: price?.pe ? price.pe.toFixed(1) : '--' },
                  { label: '换手率', value: price?.turnoverRate ? `${price.turnoverRate.toFixed(2)}%` : '--' },
                  { label: '成交量', value: price?.volume ? `${(price.volume / 1e4).toFixed(0)}万手` : '--' },
                ].map((item) => (
            ...
            <div className="bg-[#131821] border border-[#1E293B] rounded-2xl p-6 sm:p-8">
              <div className="flex items-center justify-between mb-6">
                ...
                <button
                  ...
                  disabled={downloading || !pdfData?.html}
                  className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-[#164E63]/50 text-[#06B6D4] text-xs font-medium hover:bg-[#164E63] transition-colors disabled:opacity-50"
                >
                ...
                {report?.content ? (
                  ...
                  <div>
                    ...
                    <p className="text-sm text-[#94A3B8] mb-4">
                      该标的当前五维评分为：基本面{score.fundamental}分、技术面{score.technical}分、资金面{score.capital}分、消息面{score.news}分、舆情面{score.sentiment}分。
                    </p>

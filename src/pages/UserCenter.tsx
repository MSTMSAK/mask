...
import { User, Bell, Bookmark, BarChart3, Shield, TrendingUp, TrendingDown, Clock, Zap, Star } from 'lucide-react';
            ...
            <div className="bg-[#131821] border border-[#1E293B] rounded-2xl p-6 mb-6">
              <div className="flex items-center gap-4">
                ...
                <div className="text-right">
                  <div className="data-number text-2xl text-[#06B6D4]">{portfolioValue.toLocaleString()}</div>
                  <div className={`flex items-center justify-end gap-1 text-sm font-medium ${portfolioChange >= 0 ? 'text-[#F43F5E]' : 'text-[#10B981]'}`}>
                ...
                <div className="bg-[#131821] border border-[#1E293B] rounded-2xl p-6">
                  ...
                  <div className="space-y-2">
                    {riskLevels.map((level) => (
                      <button key={level.value} onClick={() => setRisk(level.value)} className={`w-full flex items-center justify-between p-3 rounded-lg border transition-colors ${risk === level.value ? 'bg-[#164E63]/30 border-[#06B6D4]/30' : 'bg-[#0F172A] border-transparent hover:border-[#334155]'}`}>
                        <div className="text-left">
                          <div className={`text-sm font-medium ${risk === level.value ? 'text-[#06B6D4]' : 'text-[#E2E8F0]'}`}>{level.label}</div>
                          <div className="text-xs text-[#64748B]">{level.desc}</div>
                        </div>
                        {risk === level.value && (
                          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="w-5 h-5 rounded-full bg-[#06B6D4] flex items-center justify-center">
                ...
                <div className="bg-[#131821] border border-[#1E293B] rounded-2xl p-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {investmentStyles.map((style) => (
                      <button key={style} onClick={() => toggleStyle(style)} className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors ${styles.includes(style) ? 'bg-[#164E63]/40 text-[#06B6D4] border border-[#06B6D4]/30' : 'bg-[#0F172A] text-[#94A3B8] border border-[#1E293B] hover:border-[#334155]'}`}>
                        {style}
                      </button>
                    ))}
                  </div>
                ...
                <div className="bg-[#131821] border border-[#1E293B] rounded-2xl p-6">
                  ...
                  <div className="space-y-2">
                    {experienceLevels.map((level) => (
                      <button key={level.value} onClick={() => setExperience(level.value)} className={`w-full text-left p-3 rounded-lg text-sm transition-colors ${experience === level.value ? 'bg-[#164E63]/30 text-[#06B6D4] border border-[#06B6D4]/30' : 'bg-[#0F172A] text-[#94A3B8] border border-transparent hover:border-[#334155]'}`}>
                        {level.label}
                      </button>
                    ))}
                  </div>
                ...
                <div className="bg-[#131821] border border-[#1E293B] rounded-2xl p-6">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <button onClick={() => setPushEnabled(!pushEnabled)} className={`w-11 h-6 rounded-full transition-colors ${pushEnabled ? 'bg-[#06B6D4]' : 'bg-[#1E293B]'}`}>
                        <motion.div animate={{ x: pushEnabled ? 20 : 2 }} className="w-5 h-5 rounded-full bg-white shadow" />
                      </button>
                    </div>
                    <div className="flex items-center justify-between">
                      <div>
                      ...
                      <button onClick={() => setEmailEnabled(!emailEnabled)} className={`w-11 h-6 rounded-full transition-colors ${emailEnabled ? 'bg-[#06B6D4]' : 'bg-[#1E293B]'}`}>
                        <motion.div animate={{ x: emailEnabled ? 20 : 2 }} className="w-5 h-5 rounded-full bg-white shadow" />
                      </button>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-[#131821] border border-[#1E293B] rounded-2xl p-6">
                  ...
                  <div className="space-y-2">
                    {watchlistStocks.map((stock) => (
                      <div key={stock.code} className="flex items-center justify-between p-3 rounded-lg bg-[#0F172A] hover:bg-[#1E293B] transition-colors cursor-pointer">
                        <div className="flex items-center gap-3">
                          <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold ${stock.aiGrade === 'S' ? 'bg-[#06B6D4]/20 text-[#06B6D4]' : stock.aiGrade === 'A' ? 'bg-[#06B6D4]/15 text-[#06B6D4]' : 'bg-[#F59E0B]/15 text-[#F59E0B]'}`}>{stock.aiGrade}</div>
                          <div>
                        ...
                        <div className="text-right">
                          ...
                          <div className={`text-xs font-medium ${stock.changePercent >= 0 ? 'text-[#F43F5E]' : 'text-[#10B981]'}`}>
              ...
              <ScrollReveal delay={0.4}>
                <div className="bg-gradient-to-r from-[#164E63]/30 to-[#06B6D4]/10 border border-[#06B6D4]/20 rounded-2xl p-6">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <span className="px-2 py-0.5 rounded-full bg-[#06B6D4]/20 text-[#06B6D4] text-xs font-medium">已开通</span>
                  </div>
                  <p className="text-sm text-[#94A3B8] mb-4">享受完整AI深度研报、多策略融合选股、实时异动预警等全部高级功能。</p>

    ...
    <motion.div layout className="relative">
      ...
      >
        ...
        <div className="col-span-2 sm:col-span-2 text-right">
          <div className="data-number text-sm text-[#E2E8F0]">
            ...
            <div className="px-4 sm:px-6 pb-6 pt-2">
              <div className="bg-[#0F172A] rounded-xl p-4 sm:p-6 flex flex-col sm:flex-row items-center gap-6">
                <div className="flex-shrink-0">
                  <FiveDRadar data={fiveD} size={220} showLabels={true} />
                </div>
                <div className="flex-1 w-full">
                  <div className="grid grid-cols-5 gap-3 mb-4">
                    {[
                      { label: '基本面', score: fiveD.fundamental },
                      { label: '技术面', score: fiveD.technical },
                      { label: '资金面', score: fiveD.capital },
                      { label: '消息面', score: fiveD.news },
                      { label: '舆情面', score: fiveD.sentiment },
                    ].map((dim) => (
                      <div key={dim.label} className="text-center">
                        <div className="text-xs text-[#94A3B8] mb-1">{dim.label}</div>
                        <div className={`data-number text-lg ${
                          dim.score >= 85 ? 'text-[#06B6D4]' : dim.score >= 70 ? 'text-[#E2E8F0]' : dim.score >= 55 ? 'text-[#F59E0B]' : 'text-[#EF4444]'
                        }`}>{dim.score}</div>
                      </div>
                    ))}
                  </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {isLoading ? (
          ...
          <div className="bg-[#131821] border border-[#1E293B] rounded-2xl p-8 text-center">
            <p className="text-[#94A3B8] mb-4">暂无股票池数据，请先同步数据</p>
          </div>
        ) : (
          <>

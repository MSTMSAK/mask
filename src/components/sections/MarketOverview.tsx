...
export default function MarketOverview() {
  ...
  const moneyFlowData = [
    { time: '09:30', inflow: 12.5 }, { time: '10:00', inflow: 28.6 }, { time: '10:30', inflow: 35.1 },
    { time: '11:00', inflow: 42.3 }, { time: '11:30', inflow: 38.7 }, { time: '13:00', inflow: 45.2 },
    { time: '13:30', inflow: 52.1 }, { time: '14:00', inflow: 48.6 }, { time: '14:30', inflow: 55.3 }, { time: '15:00', inflow: 51.8 },
  ];
      ...
      <div className="max-w-7xl mx-auto">
        <ScrollReveal>
          <div className="mb-12 flex items-center justify-between">
            ...
            <button onClick={handleSync} disabled={syncing} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[#164E63]/50 text-[#06B6D4] text-sm hover:bg-[#164E63] transition-colors disabled:opacity-50">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
            {indicesData.map((idx) => (
              <div key={idx.name} className="bg-[#131821] border border-[#1E293B] rounded-xl p-4">
                ...
                <div className={`flex items-center gap-1 text-xs font-medium mt-1 ${idx.change >= 0 ? 'text-[#F43F5E]' : 'text-[#10B981]'}`}>
            ...
            <div className="bg-[#131821] border border-[#1E293B] rounded-2xl p-6 h-full relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <div className="h-2 bg-[#1E293B] rounded-full overflow-hidden">
                  <motion.div initial={{ width: 0 }} whileInView={{ width: `${overview?.confidence || 0}%` }} viewport={{ once: true }} transition={{ duration: 1.2, ease: 'easeOut' }} className="h-full bg-gradient-to-r from-[#06B6D4] to-[#06B6D4]/60 rounded-full" />
                </div>
              </div>
              <div className="mt-6 flex items-center gap-3 relative z-10">
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className={`text-xs font-medium ${sentimentScore > 60 ? 'text-[#F43F5E]' : sentimentScore > 40 ? 'text-[#F59E0B]' : 'text-[#10B981]'}`}>
                      {sentimentScore > 60 ? '偏乐观' : sentimentScore > 40 ? '中性' : '偏谨慎'}
                    </span>
                  </div>
                  <div className="h-1.5 bg-[#1E293B] rounded-full overflow-hidden">
                    <div className="h-full rounded-full transition-all" style={{ width: `${sentimentScore}%`, background: sentimentScore > 60 ? 'linear-gradient(90deg, #F43F5E, #F59E0B)' : 'linear-gradient(90deg, #10B981, #F59E0B)' }} />
                  </div>
                </div>
              </div>
              <div className="mt-6 grid grid-cols-3 gap-3 relative z-10">
                <div className="text-center">
                  <div className="flex items-center justify-center gap-1"><TrendingUp className="w-3.5 h-3.5 text-[#F43F5E]" /><span className="data-number text-lg text-[#F43F5E]">{overview?.upCount || 0}</span></div>
                  <span className="text-xs text-[#64748B]">上涨</span>
                </div>
                <div className="text-center">
                  <div className="flex items-center justify-center gap-1"><TrendingDown className="w-3.5 h-3.5 text-[#10B981]" /><span className="data-number text-lg text-[#10B981]">{overview?.downCount || 0}</span></div>
                ...
                <div className="text-center">
                  {overview ? (
                    ...
                    <div>
                      ...
                      <button onClick={handleSync} className="px-4 py-2 rounded-lg bg-[#164E63] text-[#06B6D4] text-sm hover:bg-[#164E63]/80 transition-colors">立即同步</button>
                    </div>
                  )}
                </div>
              </div>
